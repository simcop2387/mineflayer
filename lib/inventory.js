...
(function() {
    ...
    /**
      * Get a function that matches item(s).
      * @param {mf.ItemType OR mf.Item OR Predicate(mf.Item)} item Some sort of item to match.
      * @returns {Predicate(mf.Item)}
      */
    ...
    */
    ...
    /**
      ...
      * @param {Number} (slot) A specific slot (The first 9 slots of the inventory are viewable)
      * @returns {Boolean} True iff the contents can be viewed.
      */
    inventory.SlotArea.prototype.isViewable = function(slot) {
        if (slot !== undefined) {
        ...
        switch(this.inventoryType) {
            case inventory.InventoryType.Inventory:
                if (slot !== undefined) {
                    return slot < 9 || mf.openWindow() !== mf.WindowType.None;
                }
                return mf.openWindow() !== mf.WindowType.None;
            case inventory.InventoryType.Unique:
                return mf.openWindow() === this.windowType;
    ...
    /**
      ...
      */
    inventory.SlotArea.prototype.slotIsValid = function(slot) {
        if (slot >= this.length) {
            return false;
        }
        if (slot < 0) {
    ...
    /**
      ...
      * @param {mf.ItemType OR mf.Item OR Predicate(mf.Item)} it The function to look for matches.
      * @returns {Array} All the matching mf.Item in the inventory.
      */
    inventory.SlotArea.prototype.filtered = function(it) {
        var predicate_func = inventory.getItemPredicate(it);
    ...
    /**
      ...
      * @param {mf.ItemType OR mf.Item OR Predicate(mf.Item)} item The match-making predicate.
      * @param {Number} (count) The max number of items to look for (undefined means Infinite).  If used with comparator, the best count matching items are chosen.
      * @param {Function} (comparator) A function that takes two Items, and returns a Number (negative, 0, or positive, depending on if the first is a better choice (negative is better)).
      * @returns {Array} The list of slots for all matching items.
      */
    inventory.SlotArea.prototype.findItems = function(item, count, comparator) {
    ...
    */
    ...
    /**
      ...
      */
    inventory.SlotArea.moveHeld = function() {
        ...
        var slots = this.slotsForItem(held);
        for (var i = 0; i < slots.length; i++) {
            this.clickSlot(slots[i], mf.MouseButton.Left);
            held = mf.heldItem();
            if (held.type === mf.ItemType.NoItem) {
                return true;
            }
    ...
    /**
      * Tries to move all (matching) items from one SlotArea to another.
      * @param {SlotArea} area_into The SlotArea to move items into.
      * @param {mf.ItemType OR mf.Item OR Predicate(mf.Item)} (item) The item to match.
      */
    inventory.SlotArea.moveAll = function(area_into, item) {
        if (item === undefined) {
            item = function(it) { return true; };
        }
        if (area_into === undefined || area_into.constructor !== inventory.SlotArea) {
            return undefined;
        }
        var flag = true;
        var matching_func = inventory.getItemPredicate(item);
    ...
    /**
      ...
      * @param {mf.ItemType OR mf.Item OR Predicate(mf.Item)} item The match-making predicate.
      * @param {Function} (comparator) A function that takes two Items, and returns a Number (negative, 0, or positive, depending on if the first is a better choice (negative is better)).
      * @param {Function} (callback) A function to call iff the equipping succeeds.
      */
    inventory.SlotArea.prototype.equipItem = function(item, comparator, callback) {
        var matching_func = inventory.getItemPredicate(item);
        if (matching_func === undefined) {
            return;
        }
        ...
        if (! this.isClickable()) {
            if (this.inventoryType !== inventory.InventoryType.ActionBar) {
                return; // Inventory will be strictly not accessed, even if it's in the first 9 slots.
            }
            var slots = this.findItems(item, 1, comparator);
            if (slots.length <= 0) {
                return;
            }
            mf.selectEquipSlot(slots[0] + this.slotStart);
            callback();
            return;
        }
        var slots = this.findItems(item, 1, comparator);
        ...
        this.clickSlot(slots[0]);
        inventory.ActionBar.clickSlot(mf.selectedEquipSlot());
        if (mf.heldItem().type !== mf.ItemType.NoItem) {
            this.clickSlot(slots[0]);
        }
        callback();
    };
